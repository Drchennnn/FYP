"""
LSTM MIMO 多步输出模型 (Multi-Output Direct Multi-step)
...
def load_and_engineer_features(input_csv: Path) -> pd.DataFrame:
    ...
    for feature in ["tourism_num_lag_7_scaled", "meteo_precip_sum_scaled", "temp_high_scaled", "temp_low_scaled"]:
...
def build_sequences_mimo(df: pd.DataFrame, look_back: int, pred_steps: int = PRED_STEPS):
    feature_cols = [
    ...
    for i in range(look_back, len(df) - pred_steps + 1):
        ...
        y_list.append(target[i: i + pred_steps])
        d_list.append(dates[i])

    print(f"LSTM MIMO序列构建完成: {len(x_list)} 个样本，预测步数: {pred_steps}")
...
def create_lstm_mimo_model(look_back: int, pred_steps: int = PRED_STEPS) -> tf.keras.Model:
    """LSTM MIMO 模型：输入 (look_back, 8)，输出 (pred_steps,)"""
    ...
    print(f"LSTM MIMO 模型创建完成: 输入=({look_back}, 8), 输出={pred_steps}, 参数={model.count_params():,}")
    return model


def main() -> None:
    parser = argparse.ArgumentParser(description="LSTM MIMO 多步输出训练脚本")
    ...
    auto_run_name = f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}_lb{args.look_back}_ep{args.epochs}_lstm_mimo_8features"
    ...
    model_path = weights_dir / "lstm_mimo_jiuzhaigou.h5"
    pred_path = run_dir / "lstm_mimo_test_predictions.csv"
    history_path = run_dir / "lstm_mimo_history.csv"
    ...
    # 5. 训练
    model = create_lstm_mimo_model(args.look_back, args.pred_steps)
    callbacks = [
    ...
    try:
        save_metrics_to_files(metrics, str(run_dir), "lstm_mimo_baseline")
    except TypeError:
        pass
    ...
    evaluate_and_save_run(
        ...
        extra_meta={
            ...
            "model_architecture": f"LSTM-MIMO-128-64-32-Dense{args.pred_steps}",
    ...
    if args.save_plots:
        ...
        plt.title("LSTM MIMO: Training vs Validation Loss")
        plt.legend(); plt.tight_layout()
        plt.savefig(fig_dir / "lstm_mimo_loss.png", dpi=150); plt.close()
        ...
        plt.title("LSTM MIMO: Test Set - True vs Pred (Step 1)")
        plt.xticks(rotation=30); plt.legend(); plt.tight_layout()
        plt.savefig(fig_dir / "lstm_mimo_true_vs_pred.png", dpi=150); plt.close()
