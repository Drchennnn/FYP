...
def find_latest_run(model_key: str):
    ...
    for run_dir in os.listdir(RUNS_DIR):
        ...
        if not os.path.isdir(full):
            continue
        # match prediction csv
        csvs = glob.glob(os.path.join(full, f'{model_key}*_test_predictions.csv'))
        if not csvs:
            continue
        csv_path = csvs[0]
        ...
        # find weights
        h5s = (glob.glob(os.path.join(full, '**', '*.h5'), recursive=True) +
               glob.glob(os.path.join(full, '**', '*.keras'), recursive=True))
        if not h5s:
            continue
...
def main():
    ...
    # GRU and LSTM only (Seq2Seq uses different architecture)
