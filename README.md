...
> **最新更新（2026-04-04）**：Dashboard 升级至 v3（Apple 风格三栏布局），新增历史预测回填脚本 `scripts/backfill_predictions.py`，修复多处前后端逻辑问题，完成项目目录清理。
...
│   │   └── js/dashboard_v3.js
│   └── templates/
│       ├── dashboard_v3.html   # 当前主 Dashboard（Apple 风格）
│       └── dashboard_v2.html   # 旧版（保留兼容）
...
# 访问 http://localhost:5000/dashboard/v3  （当前主版本）
# 访问 http://localhost:5000/dashboard/v2  （旧版，保留兼容）
```
...
**功能**：
- 三模型预测曲线（GRU / LSTM / Seq2Seq+Attention），支持单独切换
- 冠军模型自动回退机制：champion → runner-up → third（按日期填补空缺）
- 离线回测 / 在线预测模式切换
- 预测窗口：1 / 3 / 7 / 14 天
- KPI 条：最新预测日期 + 综合风险 + 实时时钟（24h）
- 独立降水/温度折线图（含底部滑动窗口）
- 右侧面板：天气卡片 + 风险温度计 + 推荐出行窗口（固定跟随主图）
- 底部预测展示条：当前预测窗口逐日数据
- 节假日区域高亮
- 中英文切换 / 深色模式

**历史预测回填**：`scripts/backfill_predictions.py` 可补全模型测试集 CSV 截止日后的曲线空缺（滚动单步推断），无需重训。
...
Dashboard v3 已实现三模型对比、天气子图、风险面板、预测条等完整功能。已修复的主要问题：
- 离线模式 `forecast_end_idx` 现取所有模型列的最大覆盖日期（不再依赖单一冠军模型）
- 冠军预测空缺时自动回退到 runner → third（前端所有展示函数统一处理）
- localStorage 缓存在 `timeAxis` 为空时强制失效并重新拉取
- 天气子图移入独立卡片，增加底部滑动 dataZoom
...
| 日期 | 内容 |
|------|------|
| 2026-04-04 | 新增 `scripts/backfill_predictions.py`，修复离线预测窗口定位，修复冠军回退逻辑，清理冗余运行目录（保留最新3模型）和遗留爬虫目录 |
| 2026-04-03 | Dashboard v3 发布（Apple 风格），修复三模型曲线断层、KPI 条、天气子图独立卡片、底部预测条数据错误 |
| 2026-04-03 | 完成10年数据训练（GRU/LSTM/Seq2Seq），建立 `models/scalers/feature_scalers.pkl` |
