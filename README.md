...
- **展示**：仅显示历史回测段（测试集内的离线 CSV 预测结果）
- **数据来源**：`output/runs/*/gru_test_predictions.csv` 等
- **空白段**：测试集截止日（如4月2日）到当天之间**留白**，这是官网数据滞后 1-2 天的正常现象，**不做 backfill 填充**
- **不做在线推理**：单步模型在线推理时误差按步累积，展示价值有限

#### 多步模型（GRU MIMO / LSTM MIMO / Seq2Seq+Attention）

- **展示**：实时在线推理，连续覆盖到预测期末（无空白）
- **数据来源**：页面加载时自动调用模型权重实时推理
- **输入窗口**：最近 30 天有真实访客记录的历史数据（与测试集截止日无关）
- **输出**：`last_real_date + 1` 到 `last_real_date + 7`（7天连续）
...
MIMO GRU/LSTM / Seq2Seq：
  ...
  输出：[4/3, 4/4, 4/5, 4/6, 4/7, 4/8, 4/9] 7天连续
        ↑ 包含了单步留白的4/3、4/4，天然覆盖数据空缺段
```
